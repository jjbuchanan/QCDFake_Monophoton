...
void postAnalyzer::Loop(Long64_t maxEvents, int reportEvery)
{
  ...
  int nFilters, nHLT, nPhoCand, nWorstChIso, nMuPair, nRecoil170, nDphiPhoRecoil, nZmassWindow, nDphiJetsMET, nEleVeto;
  nFilters = nHLT = nPhoCand = nWorstChIso = nMuPair = nRecoil170 = nDphiPhoRecoil = nZmassWindow = nDphiJetsMET = nEleVeto = 0;
  ...
  for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++)
  {
    ...
    phoCand   = getQcdden(175,1.4442,1);
    ...
    if(metFilters==1536)
    {
      ...
      if(HLTPho>>12&1 == 1)
      {
        ...
        {
          nPhoCand++;
          // event_weight*=(1.013 - 0.0001168*phoEt->at(phoCand[0]));
          if(!(phoIEta->at(phoCand[0])==-24 && phoIPhi->at(phoCand[0])==141) && !(phoIEta->at(phoCand[0])==4 && phoIPhi->at(phoCand[0])==41) && !(phoIEta->at(phoCand[0])==5 && phoIPhi->at(phoCand[0])==41) && !(phoIEta->at(phoCand[0])==1 && phoIPhi->at(phoCand[0])==81) && !(phoIEta->at(phoCand[0])==4 && phoIPhi->at(phoCand[0])==21))
          {
            ...
            // NNLO_weight = event_weight*EWK_corrected_weight*NNLOCorrection(phoEt->at(phoCand[0]));

            std::vector<int> mulist = muon_veto_looseID(phoCand[0],0,0,10.0);
            std::vector<int> mulist_leading = muon_veto_tightID(phoCand[0],30.0);
            std::vector<int> mulist_subleading = muon_veto_looseID(phoCand[0],0,0,10.0);
            ...
            if(mulist.size() == 2)
            {
              ...
              for(int i=0; i<mulist_leading.size(); ++i)
              {
                for(int j=0; j<mulist_subleading.size(); ++j)
                {
                  if(muCharge->at(mulist_leading[i])*muCharge->at(mulist_subleading[j]) == -1)
                  {
                    ...
                    elelist = electron_veto_looseID(phoCand[0],mulist_leading[i],mulist_subleading[j],10.0);
                    jetveto = JetVetoDecision(phoCand[0],mulist_leading[i],mulist_subleading[j]);
                    muPairSet = true;
              ...
              if(muPairSet && subleading_passes_looseIso)
              {
                ...
                {
                  nRecoil170++;
                  fillHistos(0,NNLO_weight,phoCand[0],jetveto,lepindex_leading,lepindex_subleading,dilepton_mass,dilepton_pt,leptoMET,leptoMET_phi);
                  if(DeltaPhi(phoPhi->at(phoCand[0]),leptoMET_phi) > 0.5)
                  {
                    nDphiPhoRecoil++;
                    fillHistos(1,NNLO_weight,phoCand[0],jetveto,lepindex_leading,lepindex_subleading,dilepton_mass,dilepton_pt,leptoMET,leptoMET_phi);
                    if(dilepton_mass > 60 && dilepton_mass < 120)
                    {
                      nZmassWindow++;
                      fillHistos(2,NNLO_weight,phoCand[0],jetveto,lepindex_leading,lepindex_subleading,dilepton_mass,dilepton_pt,leptoMET,leptoMET_phi);
                      if(dPhiJetMET_veto(jetveto))
                      {
                        nDphiJetsMET++;
                        fillHistos(3,NNLO_weight,phoCand[0],jetveto,lepindex_leading,lepindex_subleading,dilepton_mass,dilepton_pt,leptoMET,leptoMET_phi);
                        if(elelist.size() == 0)
                        {
                          nEleVeto++;
                          if(uncorrectedPhoEt/leptoMET < 1.4){
                            fillHistos(4,event_weight,phoCand[0],jetveto,lepindex_leading,lepindex_subleading,dilepton_mass,dilepton_pt,leptoMET,leptoMET_phi);
                            fillHistos(5,event_weight_sidebandUp,phoCand[0],jetveto,lepindex_leading,lepindex_subleading,dilepton_mass,dilepton_pt,leptoMET,leptoMET_phi);
                            fillHistos(6,event_weight_sidebandDown,phoCand[0],jetveto,lepindex_leading,lepindex_subleading,dilepton_mass,dilepton_pt,leptoMET,leptoMET_phi);
                            fillHistos(7,event_weight_METUp,phoCand[0],jetveto,lepindex_leading,lepindex_subleading,dilepton_mass,dilepton_pt,leptoMET,leptoMET_phi);
                            fillHistos(8,event_weight_METDown,phoCand[0],jetveto,lepindex_leading,lepindex_subleading,dilepton_mass,dilepton_pt,leptoMET,leptoMET_phi);
                            fillHistos(9,event_weight_binningUp,phoCand[0],jetveto,lepindex_leading,lepindex_subleading,dilepton_mass,dilepton_pt,leptoMET,leptoMET_phi);
                            fillHistos(10,event_weight_binningDown,phoCand[0],jetveto,lepindex_leading,lepindex_subleading,dilepton_mass,dilepton_pt,leptoMET,leptoMET_phi);
                            fillHistos(11,event_weight_sieieLeft,phoCand[0],jetveto,lepindex_leading,lepindex_subleading,dilepton_mass,dilepton_pt,leptoMET,leptoMET_phi);
                            fillHistos(12,event_weight_sieieRight,phoCand[0],jetveto,lepindex_leading,lepindex_subleading,dilepton_mass,dilepton_pt,leptoMET,leptoMET_phi);
                            fillHistos(13,event_weight_templateUp,phoCand[0],jetveto,lepindex_leading,lepindex_subleading,dilepton_mass,dilepton_pt,leptoMET,leptoMET_phi);
                            fillHistos(14,event_weight_templateDown,phoCand[0],jetveto,lepindex_leading,lepindex_subleading,dilepton_mass,dilepton_pt,leptoMET,leptoMET_phi);
                            fillHistos(15,1.0,phoCand[0],jetveto,lepindex_leading,lepindex_subleading,dilepton_mass,dilepton_pt,leptoMET,leptoMET_phi);
                            runlist.push_back(run);
                            eventlist.push_back(event);
                            lumilist.push_back(lumis);
                          }
                          // if((phoSCRawE->at(phoCand[0])/TMath::CosH(phoSCEta->at(phoCand[0]))) > 700)
                          //   cout<<"phoET = "<<(phoSCRawE->at(phoCand[0])/TMath::CosH(phoSCEta->at(phoCand[0])))<<": "<<run<<":"<<lumis<<":"<<event<<endl;
                          // if(jetveto.size() > 7)
...
void postAnalyzer::fillHistos(int histoNumber, double event_weight,int index,std::vector<int> jets,int lepindex_leading,int lepindex_subleading,Double_t dilepton_mass,Double_t dilepton_pt,Double_t leptoMET,Double_t leptoMET_phi)
{
    ...
    double dPhi_leptons = DeltaPhi(muPhi->at(lepindex_leading),muPhi->at(lepindex_subleading));
